...
class Fx_data:
    ...
    def volbuilding(self):
        ...
        self.interp_estr = self.estr.rate_interpolator()
        self.sofr.data_take('USD')
        self.interp_sofr = self.sofr.rate_interpolator()
        ...
        self.vol_expiry_dates = [self.fx_calendar.advance(self.ql_today, Period(int(t * 365 + 0.5), Days)) for t in self.T]

        self.ql_surface = BlackVarianceSurface(
                                                    self.ql_today,                # reference date
                                                    self.fx_calendar,             # calendar
                                                    self.vol_expiry_dates,         # pillar dates (converted from times)
                                                    self.moneyness,              # strike vector
                                                    self.volmat.T.tolist(),                 # volatility matrix
                                                    self.day_count             # day count convention
                                                )
